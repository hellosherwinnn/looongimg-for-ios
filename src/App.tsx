...
const SCROLLBAR_WIDTH_RATIO = 0.12; // Mask right 12% to ignore scrollbar / 遮盖右侧 12% 忽略滚动条
...
export default function App() {
  ...
  const startStitching = async () => {
    ...
    try {
      ...
      // Draw the first frame completely / 完整绘制第一帧
      ...
      for (let i = 1; i < frames.length; i++) {
        ...
        if (shift > 0) {
          ...
          if (currentY + newH <= MAX_CANVAS_HEIGHT - footerH) {
            ...
            // Draw ONLY the newly revealed bottom part of the current frame / 仅绘制当前帧新露出的底部区域
      ...
      // Finalize process / 收尾处理
      video.currentTime = duration;
      await new Promise(r => video.onseeked = r);
      cCtx.drawImage(video, 0, 0); // Correctly draw to capture canvas, NOT stitched canvas! / 绘制到截取画布，而不是最终的长图画布！
